     ...
     ; ,INCON    ,INCONT   ,INPWR    ,INQQP    ,INQQPT   ,IOEQT
     ; ,IORTS    ,IOTRS    ,IOWAR    ,IUPAR    ,MAINF    ,NPARNP
     ; ,NROW     ,NUMNP    ,NZALF    ,NZCHP    ,NZCOE    ,NZQQP
     ; ,CFPARNP  ,FILENAME ,IBCOD    ,IBTCO    ,INCLK    ,NZCLK)
...
*****************************************************************************
       ...
       CHARACTER*100 LEAUX,LEEL,FILENAME*20,strFmt1*20,strFmt2*20
...
*_______________________Starts loop reading nodal coeficients
        ...
        strFmt1 = '(I'//Trim(AdjustL(strFmt1))//',9F9.0)'

        strFmt2 = ''
        Write(strFmt2,*) 4+IFmt
        strFmt2 = '(I'//Trim(AdjustL(strFmt2))//',9F9.3)'

        DO WHILE (NN.LT.NUMNP)

        LEAUX=LEEL(FILENAME,IUPAR,MAINF,NROW,INPWR)          
        READ(LEAUX,strFmt1) NN,CHP,CHPT,QQP,QQPT,ALF,ALFT,CON,CONT,CLK
C 1100   FORMAT(I<4+IFmt>,9F9.0)
