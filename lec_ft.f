...
C_______________________Starts loop through all time steps
 ...
 1000    FORMAT (I5,F10.0,I5,F10.0,4I5)
         IF (INPWR.NE.0) 
     ;           WRITE(MAINF,3100) NI,T,KT,DT1,IFLOW,ICONC,IPBFL,IPBTP
 3100    FORMAT (I5,1X,G12.5,I5,3X,G12.5,2(2X,I5),5X,2(2X,I5))
         IF (NI.EQ.I) THEN
           TIME(NI)=T
           KINT(NI)=KT
           IF (IOFLLI.NE.0 .OR. IOTRLI.NE.0) DTMXDS(NI)=DT1
           ISOLEQ(NI,1)=IFLOW
           ISOLEQ(NI,2)=ICONC
           ISOLEQ(NI,3)=IPBFL
           ISOLEQ(NI,4)=IPBTP

C------------------------- If no flow/transport problem number has been 
C------------------------- assigned, it is set equal to 1

           IF (IOEQT.NE.2 .AND. ISOLEQ(NI,3).EQ.0) ISOLEQ(NI,3)=1
           IF (IOEQT.NE.1 .AND. ISOLEQ(NI,4).EQ.0) ISOLEQ(NI,4)=1
         ELSE
           CALL ERROR(IERROR,IOWAR,MAINF,FILENAME,
     ;                  'INCORRECT ORDER TIMES',I,1,IUTIM,1,9.01)
         END IF
