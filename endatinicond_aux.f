     ...
     ; (  NUMNP      ,IFmt       ,INDEX      ,IUCAL      ,MAINF
     ;   ,IOWAR      ,INPWR      ,IERROR     ,NROW       ,FILENAME
     &   ,VAR         )
...
********************************************************************************
       ...
       CHARACTER FILENAME*20,LEEL*100,LEAUX*100,strFmt*20
       DIMENSION VAR(NUMNP)
...
C------------------------- entered into VAR. 
       ...
       Write(strFmt,*) 4+IFmt
       strFmt='(I'//Trim(AdjustL(strFmt))//',F10.0)'

       DO WHILE (I.LT.NUMNP)

          LEAUX=LEEL(FILENAME,IUCAL,MAINF,NROW,INPWR)
          READ(LEAUX,strFmt,ERR=9000) I,VV
C 1000     FORMAT(I<4+IFmt>,F10.0)
          IF (IC.EQ.1) THEN
             IF (I.NE.IC) THEN
     ...
     ;                 ' INITIAL CONDITIONS ',NROW,1,IUCAL,1,8.02)
       ...
       IF (INPWR.NE.0) CALL WRITE_ARRAYN (IFmt,MAINF,NUMNP,' ',VAR)
