     ...
     ; ,IDCONT   ,IDDMT    ,INPWR    ,IDQQP    ,IDQQPT   ,IFmt
     ; ,IOEQT    ,IOFLLI   ,IOTRLI   ,IOWAR    ,IUGRID   ,MAINF
     ; ,NROW     ,NTDMT    ,NUMNP    ,BTRA     ,CCAL     ,FILENAME
     ; ,HCAL     ,X        ,Y        ,Z        ,IDCLK)
...
********************************************************************************
       ...
       CHARACTER*100 LEEL,LEAUX,FILENAME*20,strFmt*20,strFmt2*20
...
*_______________________Card B1.2
       ...
       Write(strFmt,*)4+IFmt
       strFmt = '(I'//Trim(AdjustL(strFmt))//',6F10.0)'

       Write(strFmt2,*)4+IFmt
       strFmt2 = '(I'//Trim(AdjustL(strFmt2))//',6(1X,G10.3))'

       DO WHILE (IN.LT.NUMNP)
          IOLD=I
          I=I+1
          LEAUX=LEEL(FILENAME,IUGRID,MAINF,NROW,INPWR)      
          READ(LEAUX,strFmt,ERR=9100) IN,X1,Y1,Z1,AC,HB1,CB1

C1100      FORMAT(I<4+IFmt>,6F10.0)
          ...
          IF (I.NE.IN) THEN
             ...
             DO II=1,NN-1
                ...
                IF (INPWR.NE.0) THEN
                   ...
                   WRITE(MAINF,strFmt2) I,X(I),Y(I),Z(I),AUXAC,
     ;                               AUXHCAL,AUXCCAL
C 3100              FORMAT(I<4+IFmt>,6(1X,G10.3))
                ENDIF
                I=I+1
             ENDDO
          ENDIF
          ...
          IF (INPWR.NE.0) THEN
             ...
             WRITE(MAINF,strFmt2) I,X(IN),Y(IN),Z(IN),AUXAC,
     ;                         AUXHCAL,AUXCCAL
          ENDIF
