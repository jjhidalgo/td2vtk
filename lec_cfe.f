     ...
     ; ,INDFM    ,INDSP    ,INFOD    ,INPOR    ,INPWR    ,INSTG
     ; ,INTRA    ,IOEQT    ,IOFLSAT  ,IOTRS    ,IOWAR    ,IUPAR
     ; ,MAINF    ,NPAREL   ,NROW     ,NUMEL    ,NZARR    ,NZCOE
     ; ,NZCRD    ,NZDFM    ,NZDSP    ,NZFOD    ,NZPOR    ,NZSTG
     ; ,NZTRA    ,CFPAREL  ,FILENAME)
...
*****************************************************************************
       ...
       CHARACTER*100 LEAUX,LEEL,FILENAME*20
       Character*20::strFmt1,strFmt2,strFmt3,strFmt4,strFmt5
...
C------------------------- Initializes some auxiliar variables
       ...
       strFmt1 = '(I'//Trim(AdjustL(strFmt1))//',4F10.0)'

       strFmt2 = ''
       Write(strFmt2,*) 4+IFmt
       strFmt2 = '(I'//Trim(AdjustL(strFmt2))//',4G10.3)'

       strFmt3 = ''
       Write(strFmt3,*) 4+IFmt
       strFmt3 = '(I'//Trim(AdjustL(strFmt3))//',6F10.0)'

       strFmt4 = ''
       Write(strFmt4,*) 4+IFmt
       strFmt4 = '(I'//Trim(AdjustL(strFmt4))//',6G10.3)'

       strFmt5 = ''
       Write(strFmt5,*) 4+IFmt
       strFmt5 = '(4X,I'//Trim(AdjustL(strFmt5))//',7X,E12.5)'

               DO WHILE (NE.LT.NUMEL)  

                LEAUX=LEEL(FILENAME,IUPAR,MAINF,NROW,INPWR)          
                READ(LEAUX,strFmt1,ERR=9100) NE,TRA,STG,ARR,ARRT
C 1100           FORMAT (I<4+IFmt>,4F10.0)   
                ...
                IF (INPWR.GT.1) THEN
                   DO N=NOLD+1,NE-1
                      ...
                      IF (NZARR.NE.0) AUXART=CFPAREL(N,INARRT)
                       WRITE(MAINF,strFmt2) N,CFPAREL(N,INTRA),AUXST,
     ;                 AUXAR,AUXART
C 3300                  FORMAT (I<4+IFmt>,4G10.3)   
                   END DO
                END IF
...
*_______________________Writes last element read                
                ...
                IF (INPWR.NE.0) THEN  
                   ...
                   WRITE(MAINF,strFmt2) NE,CFPAREL(NE,INTRA),AUXST,
     ;                                  AUXAR,AUXART
                END IF
...
C------------------------- Initializes some auxiliar variables
       ...
       AUXFO=0.D0
              ...
              DO WHILE (NE.LT.NUMEL) 
                LEAUX=LEEL(FILENAME,IUPAR,MAINF,NROW,INPWR)          
                READ(LEAUX,strFmt3,ERR=9300) NE,DSP,DFM,POR,CRD,COE,FOD
C 1300           FORMAT (I<4+IFmt>,6F10.0) 
...
*_______________________Writes missing nodes
                   ...
                   IF (INPWR.GT.1) THEN
                      DO N=NOLD+1,NE-1
                         ...
                         WRITE(MAINF,strFmt4) N,AUXDS,AUXDF,
     ;                                     CFPAREL(N,INPOR),
     ;                                     AUXCR,CFPAREL(N,INCOE),AUXFO
C 3700                    FORMAT (I<4+IFmt>,6G10.3)  
...
*_______________________Writes last node read
                   ...
                   IF (INPWR.NE.0) THEN
                      ...
                      WRITE(MAINF,strFmt4) NE,AUXDS,AUXDF,
     ;                   CFPAREL(NE,INPOR),AUXCR,CFPAREL(NE,INCOE),AUXFO
                   END IF
...
*_______________________Default values
 ...
 3900       FORMAT('    ELEMENT   POROSITY COEFFICIENT')
            ...
            DO L1=1,NUMEL
              ...
              WRITE(MAINF,strFmt5)L1,CFPAREL(L1,INPOR)
C 4000         FORMAT(4X,I<4+IFmt>,7X,E12.5)
